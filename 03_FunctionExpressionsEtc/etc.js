...
//alert("Sprava pre pouzivatela");
 ...
 //var result = confirm("Je javascript fajn jazyk?");
 //console.log(result);
...
function drawLine(length,character) {
	...
	//character = character || '*';
	var riadok = "";
	for (var j=0;j<length;j++) { // po stlpcoch
...
function drawSquare(length) {
	for (var i=0;i<length;i++) { //po riadkoch
		drawLine(length);
	}
};

var drawTriangle = function(length) {
...
//	var opakovania = prompt("Kolko chces opakovani?", "5");
//
...
function promptNumber() {
	var cislo = prompt("Ake cislo ti mam povedat?", "4");
...
var digitName = function(cislo) {
	var names = ['zero', 'one', 'two', 'three', 'four', 'five'];

	var computeResult = function (digit) {
		return names[digit];
	}
